...
def require_env(
    *names: str,
    ...
    _validate_config(requirements, active_validators, environ)
    ...
    def decorate(func: Callable[P, R]) -> Callable[P, R]:
        if is_async_callable(func):
            ...
            async def async_wrapper(*args: P.args, **kwargs: P.kwargs) -> object:
                _check_environment(requirements, active_validators, environ)
...
def _check_environment(
    ...
    for name in names:
        if name not in source:
            raise EnvRequirementError(name, "is required")
        value = source[name]
