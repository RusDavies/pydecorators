...
def latest_wheel() -> Path:
    wheels = sorted(DIST_DIR.glob("pydecorators-*.whl"), key=lambda path: path.stat().st_mtime)
    if not wheels:
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="pydecorators-wheel-smoke-") as tmp:
