...
`rate_limit` applies sliding-window limits to sync and async callables. It supports global and keyed buckets, raise or block mode, injectable clocks and sleep functions for tests, process-local default storage, opt-in same-host interprocess coordination through SQLite, and opt-in Redis-backed distributed coordination. It raises `RateLimitExceeded` when a call exceeds the allowance in raise mode. See [`docs/rate_limit.md`](https://github.com/RusDavies/pydecorators/blob/master/docs/rate_limit.md) for behavior and examples.
