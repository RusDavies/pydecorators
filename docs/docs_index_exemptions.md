...
Most top-level Markdown files under `docs/` must be linked from [`docs/index.md`](https://github.com/RusDavies/pydecorators/blob/master/docs/index.md). Intentional exceptions belong in `DOCS_INDEX_EXEMPTIONS` in `tests/docs_policy_helpers.py`, with a policy test proving each exempt file exists and is deliberately excluded.
